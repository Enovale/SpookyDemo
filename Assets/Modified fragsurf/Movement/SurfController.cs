...
namespace Fragsurf.Movement
{
    public class SurfController
    {
        ...
        private void UnderwaterPhysics()
        {
            ...
            Trace waterJumpTrace = TraceBounds(playerTransform.position,
        ...
        private void LadderCheck(Vector3 colliderScale, Vector3 direction)
        {
            ...
            RaycastHit[] hits = Physics.BoxCastAll(_surfer.moveData.origin,
        ...
        public void Crouch(ISurfControllable surfer, MovementConfig config, float deltaTime)
        {
            ...
            else if (crouching)
            {
                ...
                Vector3 endPos = _surfer.moveData.origin + Vector3.up * heightDifference;
                ...
                // Uncrouch
                if (canUncrouch && crouchLerp <= 0.1f)
                {
                    crouching = false;
                    if (_surfer.collider is BoxCollider boxCollider)
                    {
